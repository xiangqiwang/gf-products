...
class CDPBrowserManager:
            ...
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                ...
                if result == 0:
                    ...
                    utils.logger.warning(
                        f"[CDPBrowserManager] CDP port {debug_port} is not accessible (error code: {result})"
                    )
                    return False
        except Exception as e:
    ...
    def is_accessible(self, host: str, port: int, timeout: float = 1.0) -> bool:
        try:
            with socket.create_connection((host, port), timeout=timeout):
                utils.logger.info(f"[CDPBrowserManager] Connectivity test to {host}:{port} successful")
                return True
        except (socket.timeout, ConnectionRefusedError) as e:
            utils.logger.warning(f"[CDPBrowserManager] Connectivity test to {host}:{port} failed: {e}")
            return False
    ...
    async def _get_browser_websocket_url(self, debug_port: int) -> str:
        ...
        try:
            async with httpx.AsyncClient() as client:
                utils.logger.info(f"[CDPBrowserManager] Attempting to get WebSocket URL from: http://127.0.0.1:{debug_port}/json/version")
                response = await client.get(
                    f"http://127.0.0.1:{debug_port}/json/version", timeout=10
                )
                utils.logger.info(f"[CDPBrowserManager] /json/version response: status={response.status_code}, headers={dict(response.headers)}")
                
                # If /json/version fails with 404, try /json
                if response.status_code == 404:
                    utils.logger.warning(f"[CDPBrowserManager] /json/version returned 404. Body: {response.text[:200]}")
                    utils.logger.info(f"[CDPBrowserManager] Trying /json fallback at: http://127.0.0.1:{debug_port}/json")
                    response = await client.get(
                        f"http://127.0.0.1:{debug_port}/json", timeout=10
                    )
                    utils.logger.info(f"[CDPBrowserManager] /json response: status={response.status_code}, headers={dict(response.headers)}")

                if response.status_code == 200:
                    data = response.json()
                    utils.logger.info(f"[CDPBrowserManager] Received 200 OK. Response data type: {type(data)}")
                    # /json/version returns a dict, /json returns a list of targets
                    if isinstance(data, list):
                        # Find the first target that has a webSocketDebuggerUrl
                        for target in data:
                            ws_url = target.get("webSocketDebuggerUrl")
                            if ws_url:
                                utils.logger.info(f"[CDPBrowserManager] Got browser WebSocket URL from /json: {ws_url}")
                                return ws_url
                    else:
                    ...
                    utils.logger.error(f"[CDPBrowserManager] webSocketDebuggerUrl not found in response: {data}")
                    raise RuntimeError("webSocketDebuggerUrl not found in response")
                else:
                    utils.logger.error(f"[CDPBrowserManager] CDP Detection failed. Final Status: {response.status_code}, Body: {response.text}")
    ...
    async def _connect_via_cdp(self, playwright: Playwright):
        ...
        """
        try:
            # Dynamically get WebSocket URL to support newer Chrome versions with GUID
            ...
            if config.CDP_CONNECT_EXISTING:
                ...
                utils.logger.info(
                    "[CDPBrowserManager] Please check your browser for a confirmation dialog and accept it (if any)"
                )
