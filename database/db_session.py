...
async def dispose_all_engines():
    """Close and dispose all cached engines"""
    for engine in _engines.values():
        await engine.dispose()
    _engines.clear()
    print("🔌 All database engines disposed.")


async def delete_all_tables(db_type: str = None):
    """Drop all tables from the database by disposing engines and deleting file"""
    if db_type is None:
        db_type = config.SAVE_DATA_OPTION
    
    # 1. First dispose all connections to release file locks
    await dispose_all_engines()
    
    # 2. If it's sqlite, we can safely delete the file now
    if db_type == "sqlite":
        db_path = sqlite_db_config['db_path']
        if os.path.exists(db_path):
            try:
                os.remove(db_path)
                print(f"🔥 SQLite database file deleted: {db_path}")
            except Exception as e:
                print(f"⚠️ Failed to delete file: {e}, falling back to drop_all")
    
    # 3. Fallback/Other DBs: Try drop_all
    engine = get_async_engine(db_type)
    if engine:
        async with engine.begin() as conn:
            await conn.run_sync(Base.metadata.drop_all)
        print(f"🗑️ All tables dropped for {db_type}")
