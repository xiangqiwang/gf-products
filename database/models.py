...
class WeiboNote(Base):
    ...
    source_keyword = Column(Text, default='', comment='来源关键词')
    user_fans_count = Column(Text, default='0', comment='作者粉丝数')
...
class XhsNote(Base):
    ...
    source_keyword = Column(Text, default='', comment='来源关键词')
    user_fans_count = Column(Text, default='0', comment='作者粉丝数')
    xsec_token = Column(Text, default='', comment='xsec_token')
...
class CollectionLog(Base):
    ...
    crawler_type = Column(String(32), comment='采集类型')
    keyword = Column(Text, comment='关键词')
    status = Column(String(32), comment='状态: running, completed, error, stopped')
    start_time = Column(String(32), comment='开始时间')
    end_time = Column(String(32), comment='结束时间')
    logs = Column(Text, comment='运行简志')
    log_file_path = Column(Text, comment='日志文件路径')
    total_count = Column(Integer, default=0, comment='采集数量')
