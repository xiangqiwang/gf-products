...
class XiaoHongShuCrawler(AbstractCrawler):
    ...
    async def search(self) -> int:
        ...
        # Map generic sort type to XHS specific sort type
        sort_mapping = {
            "latest": SearchSortType.LATEST,
            "general": SearchSortType.GENERAL,
            "popular": SearchSortType.MOST_POPULAR
        }
        xhs_sort = sort_mapping.get(config.SEARCH_SORT_TYPE, SearchSortType.GENERAL)
        ...
        for keyword in config.KEYWORDS.split(","):
            ...
            item_consecutive_skips = 0
            
            while valid_count < config.CRAWLER_MAX_NOTES_COUNT and page <= max_search_pages:
                try:
                    utils.logger.info(f"[XiaoHongShuCrawler.search] search Xiaohongshu keyword: {keyword}, page: {page}, sort: {xhs_sort.value}")
                    ...
                    if not notes_res or not notes_res.get("items"):
                        utils.logger.info(f"[XiaoHongShuCrawler.search] No more results for keyword {keyword} at page {page}")
                        break
                    ...
                    # Optimization: Process notes one by one to avoid fetching redundant details
                    # If we only need 1 note, we stop after the first successful fetch.
                    for post_item in notes_res.get("items", []):
                        if post_item.get("model_type") in ("rec_query", "hot_query"):
                            continue
                            
                        # Stop if we already have enough notes from previous pages
                        ...
                        utils.logger.info(f"[XiaoHongShuCrawler.search] Fetching detail for note {note_id} (\033[32mCurrent total: {valid_count}/{config.CRAWLER_MAX_NOTES_COUNT}\033[0m)")
                        ...
                        post_time_ms = note_detail.get("time", 0)
                        if post_time_ms:
                            dt = datetime.datetime.fromtimestamp(post_time_ms / 1000.0)
                            ...
                            if is_out_of_range:
                                item_consecutive_skips += 1
                                utils.logger.info(f"[XiaoHongShuCrawler.search] Note date {post_date_str} is out of range ({config.START_DATE}~{config.END_DATE}). Skipping ({item_consecutive_skips}/10).")
                                
                                if item_consecutive_skips >= 10:
                                    utils.logger.warning(f"[XiaoHongShuCrawler.search] Stopped early: 10 consecutive items were out of date range.")
                                    # Force outer loop to break by setting valid_count to max
                                    valid_count = config.CRAWLER_MAX_NOTES_COUNT
                                    break
                                continue
                            
                            # Reset skip counter on valid item
                            item_consecutive_skips = 0
                        ...
                        valid_count += 1
                        page_valid_count += 1
                        utils.logger.info(f"[PROGRESS] Xiaohongshu item \033[32m{valid_count}/{config.CRAWLER_MAX_NOTES_COUNT}\033[0m collected: {note_detail.get('title') or note_detail.get('desc', '')[:20]}...")
                    ...
                    if page_valid_count == 0:
                        ...
                        utils.logger.warning(f"[XiaoHongShuCrawler.search] Page {page} has 0 valid items. Consecutive skips: {consecutive_skip_pages}/3")
                    else:
                        consecutive_skip_pages = 0
                    
                    # Stop if we hit the consecutive skip threshold
                    if consecutive_skip_pages >= 3:
                        utils.logger.error(f"[XiaoHongShuCrawler.search] Stopped early: 3 consecutive pages were skipped due to date filters.")
                        break
                    ...
                    utils.logger.error(f"[XiaoHongShuCrawler.search] Error at page {page}: {e}")
                    break
