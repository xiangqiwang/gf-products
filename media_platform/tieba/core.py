...
class TieBaCrawler(AbstractCrawler):
    ...
    async def search(self) -> int:
        ...
        # Map generic sort type to Tieba specific sort type
        sort_mapping = {
            "latest": SearchSortType.TIME_DESC,
            "general": SearchSortType.RELEVANCE_ORDER,
            "popular": SearchSortType.RELEVANCE_ORDER # Tieba doesn't have a clear "popular" sort in search API, default to relevance
        }
        tieba_sort = sort_mapping.get(config.SEARCH_SORT_TYPE, SearchSortType.RELEVANCE_ORDER)
        ...
        for keyword in config.KEYWORDS.split(","):
            ...
            while valid_count < config.CRAWLER_MAX_NOTES_COUNT and page <= max_search_pages:
                try:
                    utils.logger.info(
                        f"[BaiduTieBaCrawler.search] search tieba keyword: {keyword}, page: {page}, sort: {tieba_sort.value}"
                    )
                    ...
                    if not notes_list:
                        utils.logger.info(f"[BaiduTieBaCrawler.search] No more results for keyword {keyword} at page {page}")
                        break
                    ...
                    for note_detail in notes_list:
                        ...
                        post_time_ms = note_detail.create_time
                        if post_time_ms:
                            ...
                            if config.START_DATE and post_date_str < config.START_DATE:
                                utils.logger.info(f"[BaiduTieBaCrawler.search] Note date {post_date_str} is earlier than START_DATE {config.START_DATE}. Skipping.")
                                continue
                            if config.END_DATE and post_date_str > config.END_DATE:
                                utils.logger.info(f"[BaiduTieBaCrawler.search] Note date {post_date_str} is later than END_DATE {config.END_DATE}. Skipping.")
                                continue
                        
                        note_id_list.append(note_detail.note_id)
                        valid_count += 1
                        page_valid_count += 1
                        utils.logger.info(f"[PROGRESS] Tieba item \033[32m{valid_count}/{config.CRAWLER_MAX_NOTES_COUNT}\033[0m collected: {note_detail.title[:20]}...")
                    ...
                    if page_valid_count == 0:
                        ...
                        utils.logger.warning(f"[BaiduTieBaCrawler.search] Page {page} has 0 valid items. Consecutive skips: {consecutive_skip_pages}/3")
                    else:
                        consecutive_skip_pages = 0
                    
                    # Stop if we hit the consecutive skip threshold
                    if consecutive_skip_pages >= 3:
                        utils.logger.error(f"[BaiduTieBaCrawler.search] Stopped early: 3 consecutive pages were skipped due to date filters.")
                        break
                    ...
                    utils.logger.error(
                        f"[BaiduTieBaCrawler.search] Search keywords error at page {page}: {ex}"
                    )
                    break
