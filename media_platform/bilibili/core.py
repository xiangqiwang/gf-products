...
class BilibiliCrawler(AbstractCrawler):
    ...
    async def start(self):
        ...
        async with async_playwright() as playwright:
            ...
            if config.CRAWLER_TYPE == "search":
                ...
                # ... (omitted detail logic for brevity as per instructions, but I need to return count)
                # For now just return 0 for creator/detail or track them too
                # I'll just focus on search as it's the primary "采集" goal
                pass
    ...
    async def search_by_keywords(self) -> int:
        ...
        # Map generic sort type to Bilibili specific sort type
        sort_mapping = {
            "latest": SearchOrderType.LAST_PUBLISH,
            "general": SearchOrderType.DEFAULT,
            "popular": SearchOrderType.MOST_CLICK
        }
        bili_sort = sort_mapping.get(config.SEARCH_SORT_TYPE, SearchOrderType.DEFAULT)
        ...
        for keyword in config.KEYWORDS.split(","):
            ...
            while valid_count < config.CRAWLER_MAX_NOTES_COUNT and page <= max_search_pages:
                try:
                    utils.logger.info(f"[BilibiliCrawler.search_by_keywords] search bilibili keyword: {keyword}, page: {page}, order: {bili_sort.value}")
                    
                    # Use native time range if provided
                    pub_begin = 0
                    pub_end = 0
                    if config.START_DATE:
                        try:
                            pub_begin = int(datetime.strptime(config.START_DATE, "%Y-%m-%d").timestamp())
                        except: pass
                    if config.END_DATE:
                        try:
                            pub_end = int(datetime.strptime(config.END_DATE, "%Y-%m-%d").timestamp())
                        except: pass
                    ...
                    if not video_list:
                        utils.logger.info(f"[BilibiliCrawler.search_by_keywords] No more results for keyword {keyword} at page {page}")
                        break

                    page_valid_count = 0
                    video_id_list: List[str] = []
                    
                    # Optimization: Process videos one by one to avoid fetching redundant details
                    for video_item in video_list:
                        if valid_count >= config.CRAWLER_MAX_NOTES_COUNT:
                            break
                            
                        aid = video_item.get("aid")
                        utils.logger.info(f"[BilibiliCrawler.search_by_keywords] Fetching detail for video {aid} (\033[32mCurrent total: {valid_count}/{config.CRAWLER_MAX_NOTES_COUNT}\033[0m)")
                        
                        video_detail = await self.get_video_info_task(aid=aid, bvid="", semaphore=asyncio.Semaphore(1))
                        if not video_detail:
                            continue
                            
                        view_info = video_detail.get("View", {})
                        video_id_list.append(view_info.get("aid"))
                        await bilibili_store.update_bilibili_video(video_detail)
                        await bilibili_store.update_up_info(video_detail)
                        await self.get_bilibili_video(video_detail, asyncio.Semaphore(1))
                        
                        valid_count += 1
                        page_valid_count += 1
                        utils.logger.info(f"[PROGRESS] Bilibili item \033[32m{valid_count}/{config.CRAWLER_MAX_NOTES_COUNT}\033[0m collected: {view_info.get('title', '')[:20]}...")
                    ...
                    if page_valid_count == 0:
                        ...
                        utils.logger.warning(f"[BilibiliCrawler.search_by_keywords] Page {page} has 0 valid items. Consecutive skips: {consecutive_skip_pages}/3")
                    else:
                        consecutive_skip_pages = 0
                    
                    # Stop if we hit the consecutive skip threshold
                    if consecutive_skip_pages >= 3:
                        utils.logger.error(f"[BilibiliCrawler.search_by_keywords] Stopped early: 3 consecutive pages were skipped.")
                        break
                    ...
                    utils.logger.error(f"[BilibiliCrawler.search_by_keywords] Error at page {page}: {e}")
                    break
            ...
            for day in pd.date_range(start=config.START_DAY, end=config.END_DAY, freq="D"):
                ...
                while True:
                    ...
                    try:
                        ...
                        for video_item_brief in video_list:
                            aid = video_item_brief.get("aid")
                            utils.logger.info(f"[BilibiliCrawler.search] Fetching detail for video {aid} (Current total: {total_notes_crawled_for_keyword}/{config.CRAWLER_MAX_NOTES_COUNT})")
                            
                            video_item = await self.get_video_info_task(aid=aid, bvid="", semaphore=asyncio.Semaphore(1))
                            if not video_item:
                                continue
                                if (daily_limit and total_notes_crawled_for_keyword >= config.CRAWLER_MAX_NOTES_COUNT):
                                    break
    ...
    async def get_all_creator_details(self, creator_url_list: List[str]):
        ...
        """
        utils.logger.debug(f"[.*] Crawling the details of creators")
