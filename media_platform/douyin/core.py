...
class DouYinCrawler(AbstractCrawler):
    ...
    async def search(self) -> int:
        ...
        # Map generic sort type to Douyin specific sort type
        from .field import SearchSortType
        sort_mapping = {
            "latest": SearchSortType.LATEST,
            "general": SearchSortType.GENERAL,
            "popular": SearchSortType.MOST_LIKE
        }
        dy_sort = sort_mapping.get(config.SEARCH_SORT_TYPE, SearchSortType.GENERAL)
        ...
        for keyword in config.KEYWORDS.split(","):
            ...
            while valid_count < config.CRAWLER_MAX_NOTES_COUNT and page <= max_search_pages:
                try:
                    utils.logger.info(f"[DouYinCrawler.search] search douyin keyword: {keyword}, page: {page}, sort: {dy_sort.value}")
                    ...
                    if not posts_res.get("data"):
                        utils.logger.info(f"[DouYinCrawler.search] No more results for keyword {keyword} at page {page}")
                        break
                    ...
                    for post_item in posts_res.get("data"):
                        ...
                        post_time_sec = aweme_info.get("create_time", 0)
                        if post_time_sec:
                            ...
                            if config.START_DATE and post_date_str < config.START_DATE:
                                utils.logger.info(f"[DouYinCrawler.search] Note date {post_date_str} is earlier than START_DATE {config.START_DATE}. Skipping.")
                                continue
                            if config.END_DATE and post_date_str > config.END_DATE:
                                utils.logger.info(f"[DouYinCrawler.search] Note date {post_date_str} is later than END_DATE {config.END_DATE}. Skipping.")
                                continue
                        ...
                        valid_count += 1
                        page_valid_count += 1
                        utils.logger.info(f"[PROGRESS] Douyin item \033[32m{valid_count}/{config.CRAWLER_MAX_NOTES_COUNT}\033[0m collected: {aweme_info.get('desc', '')[:20]}...")
                    ...
                    if page_valid_count == 0:
                        ...
                        utils.logger.warning(f"[DouYinCrawler.search] Page {page} has 0 valid items. Consecutive skips: {consecutive_skip_pages}/3")
                    else:
                        consecutive_skip_pages = 0
                    
                    # Stop if we hit the consecutive skip threshold
                    if consecutive_skip_pages >= 3:
                        utils.logger.error(f"[DouYinCrawler.search] Stopped early: 3 consecutive pages were skipped due to date filters.")
                        break
                    ...
                    utils.logger.error(f"[DouYinCrawler.search] Error at page {page}: {e}")
                    break
