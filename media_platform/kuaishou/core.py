            ...
            while valid_count < config.CRAWLER_MAX_NOTES_COUNT and page <= max_search_pages:
                try:
                    ...
                    if not vision_search_photo or vision_search_photo.get("result") != 1:
                        utils.logger.error(f"[KuaishouCrawler.search] search info by keyword:{keyword} result error or empty")
                        break
                    ...
                    if not feeds:
                        utils.logger.info(f"[KuaishouCrawler.search] No more results for keyword {keyword} at page {page}")
                        break
                    ...
                    for video_detail in feeds:
                        ...
                        post_time_ms = photo_info.get("timestamp", 0)
                        if post_time_ms:
                            ...
                            if config.START_DATE and post_date_str < config.START_DATE:
                                utils.logger.info(f"[KuaishouCrawler.search] Video date {post_date_str} is earlier than START_DATE {config.START_DATE}. Skipping.")
                                continue
                            if config.END_DATE and post_date_str > config.END_DATE:
                                utils.logger.info(f"[KuaishouCrawler.search] Video date {post_date_str} is later than END_DATE {config.END_DATE}. Skipping.")
                                continue
                        
                        video_id_list.append(photo_info.get("id"))
                        await kuaishou_store.update_kuaishou_video(video_item=video_detail)
                        
                        valid_count += 1
                        page_valid_count += 1
                        utils.logger.info(f"[PROGRESS] Kuaishou item \033[32m{valid_count}/{config.CRAWLER_MAX_NOTES_COUNT}\033[0m collected: {photo_info.get('caption', '')[:20]}...")
                    ...
                    if page_valid_count == 0:
                        ...
                        utils.logger.warning(f"[KuaishouCrawler.search] Page {page} has 0 valid items. Consecutive skips: {consecutive_skip_pages}/3")
                    else:
                        consecutive_skip_pages = 0
                    
                    # Stop if we hit the consecutive skip threshold
                    if consecutive_skip_pages >= 3:
                        utils.logger.error(f"[KuaishouCrawler.search] Stopped early: 3 consecutive pages were skipped due to date filters.")
                        break
                    ...
                    utils.logger.error(f"[KuaishouCrawler.search] Error at page {page}: {e}")
                    break
