...
class CrawlerManager:
    ...
    async def add_system_log(self, message: str, level: str = "info"):
        """Add a system-level log entry that will appear in the UI"""
        entry = self._create_log_entry(f"[SYSTEM] {message}", level)
                    ...
                    async with get_session() as session:
                        if session:
                            db_log = CollectionLog(
                                ...
                                keyword=config.keywords or config.specified_ids or config.creator_ids or "",
                                status="running",
                                start_time=datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                            )
                            session.add(db_log)
                            await session.flush()
                            self._current_task_id = db_log.id
                            await session.commit()
                            
                            # Create log file
                            log_dir = self._project_root / "logs"
                            log_dir.mkdir(exist_ok=True)
                            self._current_log_file = log_dir / f"task_{self._current_task_id}.log"
                            
                            # Save log file path back to DB
                            db_log.log_file_path = str(self._current_log_file.relative_to(self._project_root))
                            await session.commit()
                except Exception as db_e:
                    print(f"Failed to save collection log to DB: {db_e}")
                ...
                try:
                    async with get_session() as session:
                        if session:
                            ...
                            result = await session.execute(select(CollectionLog).where(CollectionLog.id == self._current_task_id))
                            db_log = result.scalar_one_or_none()
                            if db_log:
                                db_log.status = "stopped"
                                db_log.end_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                                db_log.total_count = self.total_collected
                                await session.commit()
                except Exception as db_e:
                    print(f"Failed to update collection log in DB: {db_e}")
    ...
    def _build_command(self, config: CrawlerStartRequest) -> list:
        ...
        if config.platform.value == "wb" and config.search_sort_type:
            # For backward compatibility or internal mapping if needed
            cmd.extend(["--search_sort_type", config.search_sort_type])
        elif config.search_sort_type:
            cmd.extend(["--search_sort_type", config.search_sort_type])
            ...
            while self.process and self.process.poll() is None:
                ...
                if line:
                    ...
                    if line:
                        ...
                        # Parse total collected count if present
                        if "Total" in line and "collected:" in line:
                            try:
                                import re
                                match = re.search(r"Total(?: items)? collected: (\d+)", line)
                                if match:
                                    self.total_collected = int(match.group(1))
                            except:
                                pass
                        
                        # Parse storage status
                        if "[Storage]" in line and "Status:" in line:
                            if "Status: New" in line: self.new_count += 1
                            elif "Status: Update" in line: self.updated_count += 1
                            elif "Status: Duplicate" in line: self.duplicate_count += 1
                        
                        # Print to main terminal for real-time visibility
                        print(f"CRAWLER: {line}")
                        
                        # Save to file
                        if self._current_log_file:
                            with open(self._current_log_file, "a", encoding="utf-8") as f:
                                f.write(f"[{datetime.now().strftime('%H:%M:%S')}] [{level.upper()}] {line}\n")
                ...
                if remaining:
                    for line in remaining.strip().split('\n'):
                        ...
                        if line:
                            ...
                            # Parse total collected count from remaining output
                            if "Total" in line and "collected:" in line:
                                try:
                                    import re
                                    match = re.search(r"Total(?: items)? collected: (\d+)", line)
                            ...
                            print(f"CRAWLER (remaining): {line}")
                    ...
                    try:
                        async with get_session() as session:
                            if session:
                                ...
                                db_log = result.scalar_one_or_none()
                                if db_log:
                                    db_log.status = "completed" if exit_code == 0 else "error"
                                    db_log.end_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                                    # Strictly use the sum of new and updated items as the ground truth
                                    final_count = self.new_count + self.updated_count
                                    db_log.total_count = final_count
                                    
                                    # Add detail summary to logs field
                                    summary = f"Total: {final_count}, New: {self.new_count}, Updated: {self.updated_count}, Duplicates: {self.duplicate_count}"
                                    # Save a snippet of logs
                                    log_text = summary + "\n" + "\n".join([l.message for l in self._logs[-50:]])
                                    db_log.logs = log_text
                                    await session.commit()
                    except Exception as db_e:
                        print(f"Failed to update collection log in DB: {db_e}")
                    self._current_task_id = None
